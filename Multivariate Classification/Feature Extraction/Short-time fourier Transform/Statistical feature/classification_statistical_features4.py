...
for subject in range(1, num_subjects + 1):
    ...
    stat_feature_vectors = np.array(feature_vectors)
    print(stat_feature_vectors.shape)

    # Reshaping the STFT features array to have a single dimension for each task
    reshaped_stft_features = stat_feature_vectors.reshape(num_grasp_tasks, num_repetitions, df.shape[0], stat_feature_vectors.shape[2])
