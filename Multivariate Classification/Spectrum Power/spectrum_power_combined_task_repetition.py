...
selected_electrode_indices = [16, 17, 18, 25, 26, 34, 41, 42, 43]
...
for grasp_task in range(eegdat.shape[2]):
    for repetition in range(eegdat.shape[3]):
        ...
        average_spectrogram = np.mean(spectrum_power_features, axis=0)
    average_spectrogram = np.mean(average_spectrogram, axis=1)

# Plotting the spectrum power features
plt.imshow((average_spectrogram), aspect='auto', origin='lower', cmap='inferno')
plt.xlabel('Repetition')
plt.ylabel('Grasp task')
plt.title('Mean Spectrum Power')
plt.colorbar(label='Power')
plt.show()
# print(average_spectrogram.shape)