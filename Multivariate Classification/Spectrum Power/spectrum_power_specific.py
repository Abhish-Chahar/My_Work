...
for grasp_task in range(eegdat.shape[2]):
    ...
    spectrum_power_spectrogram = np.zeros((eegdat.shape[0], eegdat.shape[1], len(frequency_bins)))
    ...
    for electrode in range(eegdat.shape[0]):
        for repetition in range(eegdat.shape[3]):
            ...
            spectrum_power_spectrogram[electrode, :, repetition] = mean_spectrum_power
