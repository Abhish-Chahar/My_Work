...
end_idx = np.argmax(frequency_bins >= end_freq) # Add 1 to include the end frequency bin
...
for grasp_task in range(eegdat.shape[2]):
    ...
    band_power_spectrogram = np.zeros((eegdat.shape[0], eegdat.shape[1], eegdat.shape[3]))
    ...
    plt.show()
# print(average_spectrogram.shape[0])